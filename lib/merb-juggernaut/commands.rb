...
class MerbJuggernaut::Commands
    ...
    def send_data(hash, response = false)
      ...
      hosts.each do |address|
        begin
          ...
          @socket.print(hash.to_json + ::MerbJuggernaut::CR)
          @socket.flush
          res << @socket.readline(::MerbJuggernaut::CR) if response
        ensure
  ...
  private
    ...
    def hosts
      ::MerbJuggernaut::CONFIG[:hosts].select {|h| 
        !h[:environment] or h[:environment].to_sym == Merb.env
      }
    end
